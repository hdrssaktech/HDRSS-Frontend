...
const ALL = "அனைத்தும்";
const VASTU_LABELS = ["North", "South", "East", "West", "Centre", "NE", "NW", "SE", "SW"];
...
const VaasthuPage0 = () => {
  ...
  const fetchData = () => {
    ...
    axios
      ...
      .then((res) => { setTypes(res.data); setError(null); })
  ...
  const renderItem = ({ item, index }) => (
    <TouchableOpacity
      ...
      onPress={() => navigation.navigate("VaasthuPage1", { typeId: item.id, title: item.name })}
    >
      <View style={styles.imageContainer}>
        <Image source={{ uri: item.image }} style={styles.image} resizeMode="cover" />
        <View style={styles.directionBadge}>
          <Text style={styles.directionText}>{VASTU_LABELS[index % VASTU_LABELS.length]}</Text>
        </View>
  ...
  const renderEmptyState = () => (
    <View style={styles.center}>
      <View style={styles.emptyIconWrap}>
        <Ionicons name="home-outline" size={40} color="#93210A" />
      </View>
...
const styles = StyleSheet.create({
  ...
  /* ── FOOTER — fixed height so 1-line and 2-line titles render
       identically; the brown background always fills the box,
       no card background ever shows through underneath. ── */
  cardFooter: {
    flexDirection: "row",
    alignItems: "center",
    ...
    gap: 4,
    width: "100%",
    height: 46, // fixed, not minHeight — every footer is identical regardless of title length
  },
  cardTitle: {
    flex: 1,
    fontSize: 10,
    ...
    color: "#FFFFFF",
    lineHeight: 15,
    // numberOfLines={2} in JSX caps it at 2 lines; height above already
    // reserves room for exactly 2 lines so 1-line titles don't shrink the box
  },
  arrowDot: {
    width: 16,
    height: 16,
