...
const VaasthuPage1 = () => {
  ...
  // Filter options
  const filterOptions = [ALL, ...new Set(categories.map((c) => c.title).filter(Boolean))];
  const modalOptions = filterOptions.filter((opt) =>
    opt.toLowerCase().includes(modalSearch.toLowerCase())
  );
  const filteredCategories =
    activeFilter === ALL ? categories : categories.filter((item) => item.title === activeFilter);
  ...
  const renderEmptyState = () => (
    <View style={styles.center}>
      <View style={styles.emptyIconWrap}>
        <Ionicons name="folder-open-outline" size={40} color="#93210A" />
      </View>
